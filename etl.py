...
def process_log_data(spark, input_data, output_data):
    ...
    df = df.filter(df.page == 'NextSong').select('ts', 'userId', 'song', 'artist', 'sessionId', 'location', 'userAgent')
    ...
    time_table.partitionBy('year', 'month').parquet(os.path.join(output_data, 'time/time.parquet'), 'overwrite')

    # read in song data to use for songplays table
    song_df = spark.read.json(f'{input_data}/song_data/*/*/*/*.json')

    # extract columns from joined song and log datasets to create songplays table 
    joined_df = df.join(song_df, col('df.artist') == col('song_df.artist_name'), 'inner')
...
def main():
    ...
    bucket_name = 'udinputtest'
    input_data = f"s3a://{bucket_name}"
    output_data = f"s3a://{bucket_name}/output_data"
